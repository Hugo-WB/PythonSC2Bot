...
class WorkerRushBot(sc2.BotAI):
	...
	async def on_step(self, iteration):
		...
		await self.attack(tech)
		await self.micro(tech)
	#BASIC
	...
	async def buildPylon(self):
		if (self.supply_left/self.supply_used)<0.2 and (not self.already_pending(PYLON) or (self.supply_used>120)) and not self.supply_cap == 200:
			nexuses = self.units(NEXUS).ready
			if nexuses.exists:
	...
	async def getGas(self,tech,nexuses):
		...
		if tech ==0:
			...
			gas = 0.75 * self.units(NEXUS).ready.amount
		elif tech == 2:
	...
	async def researchUpgrades(self,tech,techChoice,techChoice2):
		if self.units(UnitTypeId.TWILIGHTCOUNCIL).ready.exists:
			twilight = self.units(UnitTypeId.TWILIGHTCOUNCIL).ready.first
	
			if(self.can_afford(RESEARCH_BLINK)) and (twilight.noqueue) and (self.blink == False):
				await self.do(twilight(AbilityId.RESEARCH_BLINK))
				self.blink = True
#Attack/unit controlling
	async def attack(self,tech):
		if self.time > 480 and (self.supply_used-self.workers.amount)>100 and self.attacking == False:
			self.attacking = True
			for stalker in self.army.idle:
				await self.do(stalker.attack(self.enemy_start_locations[0]))
		if (self.supply_used-self.workers.amount)<30:
			self.attacking = False
	async def micro(self,tech):
		for unit in self.army:
			if unit.health_percentage < 0.2:
				if self.has_ability(AbilityId.RESEARCH_BLINK):
					pass
				else:
					await self.do(unit.move(unit.position.towards(self.start_location, 4)))
